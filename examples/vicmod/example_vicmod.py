...
alphas = np.concatenate([[-0.01],
                         (np.sin((e - 0.5) * math.pi) + 1) / 2,
                         [1.01]])
...
nparams = len(pnames)
params = np.random.uniform(0, 1, size=(nsamples, nparams))\
    * (pmax - pmin) + pmin
...
for iparam, param in params.iterrows():
    ...
    scalings = np.column_stack([rain / Smax,
                                rain / Smax,
                                np.ones(nval) * kb / Smax,
                                evap / Smax])
    ...
    for iax, ax in enumerate(axs):
        ...
        ax.plot(alphas, [fluxes[iax + 1](a) for a in alphas],
                "ro", label="nodes")
...
mess = f"runtime requires {tot / nsamples / nval * 3650:0.2e}"\
       + "sec for 10 years of daily simulation"
print(mess)
